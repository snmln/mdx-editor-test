...
export { getTetherAttachments } from '@devseed-ui/dropdown';
...
import { glsp, listReset, themeVal, rgba, visuallyHidden } from '@devseed-ui/theme-provider';
import { Button } from '@devseed-ui/button';
import { createOverlineStyles, Overline, Heading } from '@devseed-ui/typography';
import { iconDataURI, CollecticonChevronLeftTrailSmall, CollecticonChevronRightSmall, CollecticonChevronRightTrailSmall, CollecticonSwapHorizontal, CollecticonChevronLeftSmall, CollecticonTrashBin, CollecticonXmarkSmall, CollecticonTickSmall } from '@devseed-ui/collecticons';
import { FormGroup, FormGroupHeader, FormLabel, FormGroupBody, FormInput, FormHelper, Form, FormHelperMessage } from '@devseed-ui/form';
...
var TabsSelf = /*#__PURE__*/styled.div.withConfig({
  displayName: "Tabs__TabsSelf",
  componentId: "sc-1dbv0vw-0"
})(["display:flex;width:100%;margin-bottom:", ";align-items:flex-start;"], glsp(-1));
var TabsList = /*#__PURE__*/styled.ul.withConfig({
  displayName: "Tabs__TabsList",
  componentId: "sc-1dbv0vw-1"
})(["", " display:grid;grid-auto-columns:min-content;grid-gap:", ";li{grid-row:1;}"], listReset(), glsp());
var TabsActions = /*#__PURE__*/styled.div.withConfig({
  displayName: "Tabs__TabsActions",
  componentId: "sc-1dbv0vw-2"
})(["margin-top:", ";margin-left:auto;padding-left:", ";"], glsp(-0.5), glsp());
var TabLink = /*#__PURE__*/styled.a.withConfig({
  displayName: "Tabs__TabLink",
  componentId: "sc-1dbv0vw-3"
})(["position:relative;display:block;padding:", ";font-size:0.875rem;line-height:1rem;font-weight:", ";text-transform:uppercase;text-decoration:none;&,&:visited{color:inherit;}&::after{position:absolute;bottom:0;left:0;height:2px;width:0;content:'';background:", ";transition:all 0.16s ease-in-out 0s;", "}"], glsp(0, 0, 0.5, 0), themeVal('type.base.bold'), themeVal('color.link'), function (_ref) {
  var active = _ref.active;
  return active && css(_templateObject$1 || (_templateObject$1 = _taggedTemplateLiteral(["\n        width: 100%;\n      "])));
});
function Tabs(props) {
  var tabs = props.tabs,
      ...
      renderTabActions = props.renderTabActions;
  var tabActionsContent = null;

  if (typeof renderTabActions === 'function') {
    var tab = tabs.find(function (t) {
      return t.id === selectedTab;
    });
    var content = renderTabActions({
      tab: tab
    });
    tabActionsContent = content ? /*#__PURE__*/React.createElement(TabsActions, null, content) : null;
  }

  return /*#__PURE__*/React.createElement(TabsSelf, {
    className: className
  }, /*#__PURE__*/React.createElement(TabsList, null, tabs.map(function (t) {
    return /*#__PURE__*/React.createElement("li", {
      ...
      title: "Select ".concat(t.label, " tab"),
...
Tabs.propTypes = {
  ...
  renderTabActions: PropTypes.func,
...
function TileGroup(_ref) {
  var className = _ref.className,
      ...
      value = _ref.value,
      valueType = _ref.valueType,
      tileProps = _objectWithoutProperties$7(_ref, _excluded$8);
...
function Month(_ref) {
  var classes = _ref.classes,
      ...
      _ref$formatMonthYear = _ref.formatMonthYear,
      formatMonthYear$1 = _ref$formatMonthYear === void 0 ? formatMonthYear : _ref$formatMonthYear,
      otherProps = _objectWithoutProperties$4(_ref, _excluded$5);
...
function Days(props) {
  ...
  var showFixedNumberOfWeeks = props.showFixedNumberOfWeeks,
      ...
      otherProps = _objectWithoutProperties$2(props, _excluded$3);
...
function MonthView(props) {
  ...
  var _props$calendarType = props.calendarType,
      ...
      childProps = _objectWithoutProperties$1(props, _excluded$2);
...
var DATE_FORMAT_MONTH = 'MMM yyyy';
var DATE_FORMAT_FULL = 'MMM do, yyyy';
/**
...
var formatDate = function formatDate(date, _ref) {
  var view = _ref.view;
  if (!date) return '';
...
var formatRangeLabel = function formatRangeLabel(range) {
  var _ref2 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      view = _ref2.view;

  var start = range.start,
      end = range.end; // Format the label in a pretty way as to not repeat values if the month or
  // year are the same.
  // Examples:
  // Jan 1st, 2020 - Dec 31st, 2021
  // Jan 1st - Dec 31st, 2020
  ...
  var dateFormat = view === 'decade' ? DATE_FORMAT_YEAR : view === 'year' ? DATE_FORMAT_MONTH : DATE_FORMAT_FULL;

  if (start) {
    var startStr = format(start, dateFormat);

    if (!end) {
      return "".concat(startStr, " \u2014 end");
    }

    var endStr = format(end, dateFormat);
    ...
    if (isSameMonth(start, end) && isSameYear(start, end)) {
      ...
      return "".concat(format(start, 'MMM dd'), "-").concat(format(end, 'dd, yyyy'));
    } else if (isSameYear(start, end)) {
      // decade view does not matter because being the same year it gets
      // captured above.
      if (view === 'year') {
        return "".concat(format(start, 'MMM'), " \u2014 ").concat(endStr);
      }

      return "".concat(format(start, 'MMM do'), " \u2014 ").concat(endStr);
    } else {
      return "".concat(startStr, " \u2014 ").concat(endStr);
    }
  } else if (end) {
    return "From start \u2014 ".concat(format(end, dateFormat));
  }
...
var formatDateLabel = function formatDateLabel(value, _ref3) {
  var view = _ref3.view,
      isRange = _ref3.isRange;
  return isRange ? formatRangeLabel(value, {
    view: view
...
var parseDateStr = function parseDateStr(str) {
  ...
  var dateStr = str.trim().toLowerCase(); // Date from which to get base values for date-fns
  ...
  if (view === 'decade') {
    ...
    if (date) {
      ...
      var cleanString = dateStr.match(/[0-9]{4}/) ? // Starts with year. Add at the end.
      "".concat(dateStr.replace(/-/g, ' '), " 01") : // Add at the beginning on other cases.
      "01 ".concat(dateStr.replace(/-/g, ' '));
      var time = Date.parse(cleanString);
      date = new Date(time);
    }
    ...
    //
    var isoYearOrMonth = dateStr.match(/^([0-9]{4})(?:-([0-9]{1,2}))?$/);
    var isoMatch = dateStr.match(/^([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})/);
    ...
    if (isoYearOrMonth) {
      ...
      var _isoYearOrMonth = _slicedToArray(isoYearOrMonth, 3),
          y = _isoYearOrMonth[1],
          _isoYearOrMonth$ = _isoYearOrMonth[2],
          m = _isoYearOrMonth$ === void 0 ? '01' : _isoYearOrMonth$;

      _cleanString = "".concat(y, "/").concat(m, "/01");
    } else if (isoMatch) {
...
var compareDates = function compareDates(a, b, _ref5) {
  var view = _ref5.view;
  // Comparison can be done by they string representation, based on view.
  var formatYear = 'yyyy';
  var formatMonth = 'yyyy/MM';
...
var prepareInputDates = function prepareInputDates(value) {
  var _ref6 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
      isRange = _ref6.isRange,
      view = _ref6.view;
  ...
  if (!isRange) {
    ...
    var d = value instanceof Date ? value : value.start instanceof Date ? value.start : null;
    return {
      start: d,
      end: d
    };
  } else {
    var dStart = value.start && value.start instanceof Date ? value.start : null;
    var dEnd = value.end && value.end instanceof Date ? value.end : null; // If we have both dates, ensure they're in order.

    if (dStart && dEnd) {
      return compareDates(dStart, dEnd, {
        view: view
      }) === -1 ? {
        start: dStart,
        end: dEnd
...
var useSafeState = function useSafeState(initialValue) {
...
var computeDisabledDates = function computeDisabledDates(dateList, _ref7) {
  var view = _ref7.view;
  ...
  // 2022     --> Set()
  // 2022/01  --> Set()
  var dateSet = dateList.reduce(function (acc, date) {
    // How the dates are grouped depends on the view:
    var groupFormat = {
      month: 'yyyy/MM/dd',
      year: 'yyyy/MM',
      decade: 'yyyy'
    }[view]; // Use a formatted date so the Set() can compare them uniquely.

    var formattedDate = format(date, groupFormat); // Dates that share a month.

    var m = format(date, 'yyyy/MM');
    if (!acc[m]) acc[m] = new Set();
    acc[m].add(formattedDate); // Dates that share a year.

    var y = format(date, 'yyyy');
    if (!acc[y]) acc[y] = new Set();
    acc[y].add(formattedDate);
    return acc;
  }, {});
  var toDisable = Object.entries(dateSet).reduce(function (acc, _ref8) {
    ...
    if (key.length === 4) {
      var expectedSize = {
        month: getDayOfYear(new Date("".concat(key, "/12/31"))),
        year: 12,
        decade: 1
      }[view]; // If we all the expected items the the year should be disabled.
      ...
      if (theSet.size === expectedSize) {
        acc.years.push(new Date("".concat(key, "/01/01")));
        return acc;
      }
    ...
    if (key.length === 7) {
      var _expectedSize = {
        month: endOfMonth(new Date("".concat(key, "/01"))).getDate(),
        year: 1,
        decade: 1
      }[view]; // If we have items for the whole month it should be disabled.

      if (theSet.size === _expectedSize) {
        acc.months.push(new Date("".concat(key, "/01")));
        return acc;
      }
    }
  ...
  switch (view) {
    case 'month':
      return _objectSpread2(_objectSpread2({}, toDisable), {}, {
        days: dateList
      });
...
var useDateNotAllowed = function useDateNotAllowed(datesToRestrict, restrictMode, _ref10) {
  var view = _ref10.view;
  var restrictDateByUnit = useMemo(function () {
    if (!(datesToRestrict !== null && datesToRestrict !== void 0 && datesToRestrict.length)) return null;

    if (!['month', 'year', 'decade'].includes(view)) {
      throw new Error("Invalid view value: ".concat(view));
    }
    ...
    if (restrictMode === 'disable') {
      return computeDisabledDates(datesToRestrict, {
        view: view
      });
    }

    throw new Error("Invalid restrictMode value: ".concat(restrictMode));
  }, [restrictMode, datesToRestrict, view]);
  return useCallback(function (_ref11) {
    var date = _ref11.date,
        currentView = _ref11.view;
    // Don't do anything if there are no dates.
    if (!restrictDateByUnit) return false;
    var isEnabling = restrictMode === 'enable'; // Map the calendar view to our dates structure. The calendar view refers
    ...
    var dateFound = restrictDateByUnit[v].some(function (d) {
      return compareDates(date, d, {
        view: currentView
      }) === 0;
    });
...
var iconAsPseudoBackground = function iconAsPseudoBackground(icon) {
...
var CalendarStyled = /*#__PURE__*/styled(ReactCalendar).withConfig({
  displayName: "Calendar__CalendarStyled",
  componentId: "sc-14736lo-0"
})(["--rangeSelectBorderColor:", ";display:grid;grid-gap:", ";.react-calendar__tile,.react-calendar__navigation__label,.react-calendar__navigation__arrow{border:0;background:none;cursor:pointer;border-radius:", ";color:", ";transition:all 0.24s ease 0s;&:focus{outline:0;}}.react-calendar__navigation{display:flex;> *{line-height:1.5rem;}> [disabled]{pointer-events:none;opacity:0.64;}&__prev-button{margin-left:auto;}&__arrow{width:1.5rem;height:1.5rem;order:2;&:hover{background:", ";}&:active{background:", ";}span{", "}&.react-calendar__navigation__prev-button::before{", "}&.react-calendar__navigation__prev2-button::before{", "}&.react-calendar__navigation__next-button::before{", "}&.react-calendar__navigation__next2-button::before{", "}}&__label{display:flex;justify-content:center;align-items:center;flex-grow:0 !important;padding:", ";font-weight:", ";&::after{", " margin-left:", ";}&:hover{background:", ";}&:active{background:", ";}}}.react-calendar__month-view__weekdays,.react-calendar__month-view__days,.react-calendar__year-view__months,.react-calendar__decade-view__years{display:grid !important;grid-gap:", ";text-align:center;}.react-calendar__month-view__weekdays,.react-calendar__month-view__days{grid-template-columns:repeat(7,1fr);}.react-calendar__year-view__months{grid-template-columns:repeat(3,1fr);}.react-calendar__decade-view__years{grid-template-columns:repeat(4,1fr);}.react-calendar__month-view__weekdays{", " font-size:0.75rem;abbr{text-decoration:none;}&__weekday{flex-basis:0 !important;max-width:none !important;overflow:visible !important;margin-bottom:", ";}}.react-calendar__month-view__days__day--neighboringMonth{color:", ";}.react-calendar__tile{position:relative;flex-basis:0 !important;max-width:none !important;overflow:visible !important;line-height:1.25rem;padding:", ";height:2rem;border-radius:", ";border:2px dashed transparent;&:disabled{opacity:0.48;pointer-events:none;}&--active,&--hasActive{background:", ";color:#ffffff;}&--now{z-index:10;&::before{position:absolute;z-index:1;top:0;left:50%;transform:translate(-50%,-50%);background:", ";border-radius:", ";box-shadow:0 0 0 2px #ffffff;height:0.5rem;width:0.5rem;content:'';pointer-events:none;}}}.react-calendar__tile--range{background:", ";color:inherit;}.react-calendar__month-view__days{.react-calendar__tile--range{&:not(.react-calendar__tile--rangeStart):not(.react-calendar__tile--rangeEnd){&:not(:nth-child(7n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}&:not(:nth-child(7n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}}.react-calendar__tile--rangeStart:not(.react-calendar__tile--rangeEnd):not(:nth-child(7n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}.react-calendar__tile--rangeEnd:not(.react-calendar__tile--rangeStart):not(:nth-child(7n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}.react-calendar__year-view__months{.react-calendar__tile--range{&:not(.react-calendar__tile--rangeStart):not(.react-calendar__tile--rangeEnd){&:not(:nth-child(3n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}&:not(:nth-child(3n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}}.react-calendar__tile--rangeStart:not(.react-calendar__tile--rangeEnd):not(:nth-child(3n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}.react-calendar__tile--rangeEnd:not(.react-calendar__tile--rangeStart):not(:nth-child(3n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}.react-calendar__decade-view__years{.react-calendar__tile--range{&:not(.react-calendar__tile--rangeStart):not(.react-calendar__tile--rangeEnd){&:not(:nth-child(4n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}&:not(:nth-child(4n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}}.react-calendar__tile--rangeStart:not(.react-calendar__tile--rangeEnd):not(:nth-child(4n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}.react-calendar__tile--rangeEnd:not(.react-calendar__tile--rangeStart):not(:nth-child(4n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}.react-calendar__tile--rangeStart,.react-calendar__tile--rangeEnd{background:", ";color:#ffffff;}.react-calendar__tile:hover,.react-calendar__tile--active:hover{background:", " !important;color:inherit !important;}&.react-calendar--selectRange{.react-calendar__tile{&--rangeStart:hover{background:", " !important;}&--hoverStart:not(.react-calendar__tile--rangeStart){border-left-color:var(--rangeSelectBorderColor) !important;}&--hoverEnd:not(.react-calendar__tile--rangeStart){border-right-color:var(--rangeSelectBorderColor) !important;}}.react-calendar__month-view__days{.react-calendar__tile{&--hover:not(.react-calendar__tile--rangeStart){border-color:var(--rangeSelectBorderColor);&:not(:nth-child(7n + 0)){border-right-color:transparent;}&:not(:nth-child(7n + 1)){border-left-color:transparent;}}}.react-calendar__tile--hoverStart:not(.react-calendar__tile--hoverEnd):not(:nth-child(7n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}.react-calendar__tile--hoverEnd:not(.react-calendar__tile--hoverStart):not(:nth-child(7n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}.react-calendar__year-view__months{.react-calendar__tile{&--hover:not(.react-calendar__tile--rangeStart){border-color:var(--rangeSelectBorderColor);&:not(:nth-child(3n + 0)){border-right-color:transparent;}&:not(:nth-child(3n + 1)){border-left-color:transparent;}}}.react-calendar__tile--hoverStart:not(.react-calendar__tile--hoverEnd):not(:nth-child(3n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}.react-calendar__tile--hoverEnd:not(.react-calendar__tile--hoverStart):not(:nth-child(3n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}.react-calendar__decade-view__years{.react-calendar__tile{&--hover:not(.react-calendar__tile--rangeStart){border-color:var(--rangeSelectBorderColor);&:not(:nth-child(4n + 0)){border-right-color:transparent;}&:not(:nth-child(4n + 1)){border-left-color:transparent;}}}.react-calendar__tile--hoverStart:not(.react-calendar__tile--hoverEnd):not(:nth-child(4n + 0)){border-top-right-radius:0;border-bottom-right-radius:0;}.react-calendar__tile--hoverEnd:not(.react-calendar__tile--hoverStart):not(:nth-child(4n + 1)){border-top-left-radius:0;border-bottom-left-radius:0;}}}"], rgba(themeVal('color.link'), 0.32), glsp(0.75), themeVal('shape.rounded'), themeVal('type.base.color'), themeVal('color.base-100a'), themeVal('color.base-200a'), visuallyHidden(), iconAsPseudoBackground(CollecticonChevronLeftSmall), iconAsPseudoBackground(CollecticonChevronLeftTrailSmall), iconAsPseudoBackground(CollecticonChevronRightSmall), iconAsPseudoBackground(CollecticonChevronRightTrailSmall), glsp(0, 0.5), themeVal('type.base.bold'), iconAsPseudoBackground(CollecticonSwapHorizontal), glsp(0.5), themeVal('color.base-100a'), themeVal('color.base-200a'), glsp(0.25, 0), createOverlineStyles, glsp(0.25), rgba(themeVal('color.base'), 0.48), glsp(0, 0.5), themeVal('shape.rounded'), themeVal('color.link'), themeVal('color.link'), themeVal('shape.rounded'), rgba(themeVal('color.link'), 0.32), themeVal('color.link'), rgba(themeVal('color.link'), 0.16), rgba(themeVal('color.link'), 0.32));
function Calendar(props) {
  var value = props.value,
      ...
      max = props.max,
      onChange = props.onChange,
      datesToRestrict = props.datesToRestrict,
      restrictMode = props.restrictMode; // Convert from array to object
  ...
  }, [isRange, onChange]); // and vice versa...
  ...
  var isTileDisabled = useDateNotAllowed(datesToRestrict, restrictMode, {
    view: view
  }); // Default start date is today, unless it is outside the min/max domain

  var startDate = value.end || value.start || new Date();

  if (min && compareDates(startDate, min, {
    view: view
  }) === -1) {
    startDate = min;
  }

  if (max && compareDates(startDate, max, {
    view: view
  }) === 1) {
    startDate = max;
  }
  ...
  return /*#__PURE__*/React.createElement(CalendarStyled, {
    ...
    prevLabel: /*#__PURE__*/React.createElement("span", null, "Previous"),
    prev2Label: null,
    nextLabel: /*#__PURE__*/React.createElement("span", null, "Next"),
    next2Label: null,
    ...
    formatShortWeekday: function formatShortWeekday(l, date) {
...
Calendar.propTypes = {
  ...
  view: PropTypes.oneOf(['month', 'year', 'decade']),
  isRange: PropTypes.bool,
  onChange: PropTypes.func,
  datesToRestrict: PropTypes.arrayOf(PropTypes.instanceOf(Date)),
  restrictMode: PropTypes.oneOf(['enable', 'disable'])
};
...
function StressedField(props) {
  ...
  var _useSafeState = useSafeState(false),
      ...
      errored = _useSafeState2[0],
      setErrored = _useSafeState2[1];

  var _useSafeState3 = useSafeState(value),
      _useSafeState4 = _slicedToArray(_useSafeState3, 2),
      draftValue = _useSafeState4[0],
      setDraftValue = _useSafeState4[1]; // Update internal state (draft), when incoming value changes.
...
function StressedFormGroupInput(props) {
  var id = props.id,
      name = props.name,
      label = props.label,
      value = props.value,
      ...
      inputVariation = props.inputVariation,
      placeholder = props.placeholder,
      onChange = props.onChange,
      helper = props.helper;
  return /*#__PURE__*/React.createElement(FormGroup, null, /*#__PURE__*/React.createElement(FormGroupHeader, null, /*#__PURE__*/React.createElement(FormLabel, {
    htmlFor: id
  }, label)), /*#__PURE__*/React.createElement(FormGroupBody, null, /*#__PURE__*/React.createElement(StressedField, {
    value: value,
    validate: validate,
    onChange: onChange,
    render: function render(_ref) {
      ...
      return /*#__PURE__*/React.createElement(FormInput, _extends$d({
        ref: ref,
        type: inputType,
  ...
  }), helper && /*#__PURE__*/React.createElement(FormHelper, null, helper)));
}
StressedFormGroupInput.propTypes = {
  ...
  inputType: PropTypes$1.oneOf(['number', 'text']),
  inputSize: PropTypes$1.string,
  inputVariation: PropTypes$1.string,
  placeholder: PropTypes$1.oneOfType([PropTypes$1.string, PropTypes$1.number]),
  validate: PropTypes$1.func,
  onChange: PropTypes$1.func,
  helper: PropTypes$1.node
};

function DirectDateInput(props) {
  var id = props.id,
      isRange = props.isRange,
      ...
      max = props.max,
      datesToRestrict = props.datesToRestrict,
      restrictMode = props.restrictMode;
  ...
  var isDateDisallowed = useDateNotAllowed(datesToRestrict, restrictMode, {
    view: view
  ...
  var onValChangeSingle = function onValChangeSingle(v, setV) {
    var date = parseDateStr(v, {
      view: view
    });
    !!date && setV(formatDate(date, {
      view: view
    }));
    onChange({
      start: date,
  ...
  var onValChangeStart = function onValChangeStart(v, setV) {
    var date = parseDateStr(v, {
      view: view
    });
    !!date && setV(formatDate(date, {
  ...
  var onValChangeEnd = function onValChangeEnd(v, setV) {
    var date = parseDateStr(v, {
      view: view
    });
    !!date && setV(formatDate(date, {
  ...
  var onValidate = function onValidate(field, v) {
    var date = parseDateStr(v, {
      view: view
    }); // Clean the errors.
    ...
    setErrors(function (_ref) {
      _ref[field];
          var rest = _objectWithoutProperties$9(_ref, [field].map(_toPropertyKey));
    ...
    if (isDateDisallowed({
      ...
      setErrors(function (e) {
        return _objectSpread2(_objectSpread2({}, e), {}, _defineProperty$d({}, field, 'disallowed'));
      });
      return false;
    }
    ...
    if (min && compareDates(date, min, {
      ...
      setErrors(function (e) {
        return _objectSpread2(_objectSpread2({}, e), {}, _defineProperty$d({}, field, 'min-date'));
      });
      return false;
    }

    if (max && compareDates(date, max, {
      view: view
    }) === 1) {
      setErrors(function (e) {
        return _objectSpread2(_objectSpread2({}, e), {}, _defineProperty$d({}, field, 'max-date'));
      });
      return false;
    }
    ...
    if (isRange) {
      ...
      // that date.
      if (field === 'dateStart' && end && compareDates(date, end, {
        view: view
      }) === 1) {
        setErrors(function (e) {
          return _objectSpread2(_objectSpread2({}, e), {}, _defineProperty$d({}, field, 'after-end'));
        });
        return false;
      ...
      if (field === 'dateEnd' && start && compareDates(date, start, {
        view: view
      }) === -1) {
        setErrors(function (e) {
          return _objectSpread2(_objectSpread2({}, e), {}, _defineProperty$d({}, field, 'before-start'));
        });
        return false;
      }
    }
  ...
  var renderError = function renderError(field) {
    ...
    return /*#__PURE__*/React.createElement(FormHelperMessage, {
      invalid: true
    }, errors[field] === 'min-date' && "Date must be after or on: ".concat(formatDate(min, {
      view: view
    })), errors[field] === 'max-date' && "Date must be before or on: ".concat(formatDate(max, {
      view: view
    })), errors[field] === 'after-end' && "Date must be before end date: ".concat(formatDate(end, {
      view: view
    })), errors[field] === 'before-start' && "Date must be after start date: ".concat(formatDate(start, {
      view: view
    })), errors[field] === 'disallowed' && "The inserted date is not allowed");
  };

  var placeholderDate = new Date();
  return isRange ? /*#__PURE__*/React.createElement(Form, null, /*#__PURE__*/React.createElement("button", {
    type: "submit",
    disabled: true,
    style: {
      display: 'none'
    ...
    "aria-hidden": "true"
  }), /*#__PURE__*/React.createElement(StressedFormGroupInput, {
    inputType: "text",
    id: "".concat(id, "-date-start"),
    name: "".concat(id, "-date-start"),
    label: "Start date",
    ...
    placeholder: formatDate(placeholderDate, {
      view: view
    }),
    helper: renderError('dateStart')
  }), /*#__PURE__*/React.createElement(StressedFormGroupInput, {
    inputType: "text",
    id: "".concat(id, "-date-end"),
    name: "".concat(id, "-date-end"),
    label: "End date",
...
var _excluded = ["id", "className", "isRange", "view", "min", "max", "value", "onCancel", "onConfirm", "onDateChange", "renderTriggerElement", "direction", "alignment", "onChange", "renderAdditionalContent", "datesToRestrict", "restrictMode", "isClearable"];
var DropdownDatePicker = /*#__PURE__*/styled(Dropdown).withConfig({
  displayName: "Picker__DropdownDatePicker",
  componentId: "sc-1wc2xso-0"
})(["&&{max-width:none;width:auto;min-width:20rem;display:grid;grid-gap:", ";}"], glsp());
var DropHeader = /*#__PURE__*/styled.header.withConfig({
  displayName: "Picker__DropHeader",
  componentId: "sc-1wc2xso-1"
})(["display:grid;grid-gap:", ";box-shadow:0 1px 0 0 ", ";margin:", ";padding:", ";"], glsp(), themeVal('color.base-100a'), glsp(-1, -1, 0, -1), glsp());
var DropHeadline = /*#__PURE__*/styled.div.withConfig({
  displayName: "Picker__DropHeadline",
  componentId: "sc-1wc2xso-2"
})(["display:grid;align-items:center;grid-template-columns:1fr 1fr;"]);
var DropFooterActions = /*#__PURE__*/styled.div.withConfig({
  displayName: "Picker__DropFooterActions",
  componentId: "sc-1wc2xso-3"
})(["display:flex;gap:", ";box-shadow:0 -1px 0 0 ", ";justify-content:flex-end;margin:", ";padding:", ";"], glsp(0.5), themeVal('color.base-100a'), glsp(0, -1, -1, -1), glsp());
var ClearButton = /*#__PURE__*/styled(Button).withConfig({
  displayName: "Picker__ClearButton",
  componentId: "sc-1wc2xso-4"
})(["margin-right:auto;"]);
var DropTitle = /*#__PURE__*/styled(Heading).withConfig({
  displayName: "Picker__DropTitle",
  componentId: "sc-1wc2xso-5"
})(["margin:0;grid-column:1 / -1;"]);
var DropBody = /*#__PURE__*/styled.div.withConfig({
  displayName: "Picker__DropBody",
  componentId: "sc-1wc2xso-6"
})([""]);
var EMPTY_DATE = {
  start: null,
  end: null
...
var DatePicker = function DatePicker(props) {
  var id = props.id,
      ...
      rest = _objectWithoutProperties$9(props, _excluded);

  var value = prepareInputDates(rawVal, {
    isRange: isRange,
    view: view
  });
  var tabs = [{
  ...
  var _useSafeState = useSafeState(tabs[0].id),
      _useSafeState2 = _slicedToArray(_useSafeState, 2),
      activeTab = _useSafeState2[0],
      setActiveTab = _useSafeState2[1];

  var _useSafeState3 = useSafeState(value),
      _useSafeState4 = _slicedToArray(_useSafeState3, 2),
      draftDate = _useSafeState4[0],
      setDraftDate = _useSafeState4[1]; // Update value. Use a string for comparison.


  var inputEncoded = JSON.stringify(value);
  useEffect(function () {
    setDraftDate(value); // value and inputEncoded and connected. There's no need to track value.

    /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [inputEncoded]);
  var onCancelButtonClick = useCallback(function () {
    // Reset to draft.
    setDraftDate(value);
    onCancel && onCancel(); // value and inputEncoded and connected. There's no need to track value.

    /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [onCancel, inputEncoded]);
  var onConfirmButtonClick = useCallback(function () {
    onConfirm(draftDate);
  }, [onConfirm, draftDate]);
  var triggerElement = typeof renderTriggerElement === 'function' ? function (props) {
    return renderTriggerElement(props, formatDateLabel(value, {
      view: view,
      isRange: isRange
    }));
  } :
  ...
  function (props) {
    ...
    }, props), formatDateLabel(value, {
      view: view,
      isRange: isRange
    }));
  };
  var onDraftDateChange = useCallback(function (value) {
    setDraftDate(value);
  ...
  /* eslint-disable-next-line react-hooks/exhaustive-deps */
  [onDateChange]);
  var onClearButtonClick = useCallback(function () {
    setDraftDate(EMPTY_DATE); // setDraftDate will never change.

    /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, []);
  return /*#__PURE__*/React.createElement(DropdownDatePicker, _extends$d({
    className: className,
    id: id,
    alignment: alignment,
    direction: direction,
    ...
    onChange: function onChange(isOpen) {
      ...
      !isOpen && setActiveTab(tabs[0].id); // Allow implementers to extend dropdown functionality

      typeof _onChange === 'function' && _onChange(isOpen);
    }
  }, rest), /*#__PURE__*/React.createElement(DropHeader, null, /*#__PURE__*/React.createElement(DropHeadline, null, /*#__PURE__*/React.createElement(Overline, null, isRange ? 'Select date range' : 'Select date'), /*#__PURE__*/React.createElement(DropTitle, {
    size: "xsmall",
    "data-testid": "devseedui-current-date"
  ...
  })), /*#__PURE__*/React.createElement(DropBody, null, activeTab === 'calendar' && /*#__PURE__*/React.createElement(Calendar, {
    id: "".concat(id, "-calendar"),
    value: draftDate,
  ...
  }), activeTab === 'input' && /*#__PURE__*/React.createElement(DirectDateInput, {
    id: "".concat(id, "-input"),
    value: draftDate,
    onChange: onDraftDateChange,
    isRange: isRange,
    ...
    tab: activeTab
  })), /*#__PURE__*/React.createElement(DropFooterActions, null, isClearable && /*#__PURE__*/React.createElement(ClearButton, {
    variation: "base-text",
    size: "small",
    onClick: onClearButtonClick,
    "aria-label": "Clear date selection"
  }, /*#__PURE__*/React.createElement(CollecticonTrashBin, {
  ...
  }, /*#__PURE__*/React.createElement(CollecticonXmarkSmall, {
  ...
  }, /*#__PURE__*/React.createElement(CollecticonTickSmall, {
    title: "Save date selection",
    meaningful: true
  }), " Save")));
};
...
DatePicker.propTypes = {
  ...
  min: PropTypes.instanceOf(Date),
  max: PropTypes.instanceOf(Date),
  value: PropTypes.oneOfType([PropTypes.instanceOf(Date), PropTypes.shape({
    start: PropTypes.instanceOf(Date),
    end: PropTypes.instanceOf(Date)
  })]),
  ...
  renderTriggerElement: PropTypes.func,
